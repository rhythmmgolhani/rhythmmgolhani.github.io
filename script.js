...
function showPage(id) {
  document.querySelectorAll('.page , .page_merge').forEach(p => p.classList.remove('active'));
...
function toggleMenu(suffix) {
  document.getElementById('mobile-menu' + suffix).classList.toggle('open');
  document.getElementById('hamburger-btn' + suffix).classList.toggle('open');
}
function closeMenu(suffix) {
  document.getElementById('mobile-menu' + suffix).classList.remove('open');
  document.getElementById('hamburger-btn' + suffix).classList.remove('open');
}
// ============================================================
// COUNTDOWN TIMER
...
// document.documentElement.style.setProperty('--bg-img', `url(${IMG_BACKGROUND_FRONT})`);
...
function startBinAutoRotate() {
  if (binAutoTimer) return;
  binAutoTimer = setInterval(switchDustbin, 4000);
}
startBinAutoRotate();
